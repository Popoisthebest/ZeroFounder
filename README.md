...
- `disallowed_file` / `deleted_file` / `too_many_files`: 행동별 허용 파일 목록과 파일 수, 삭제 여부를 확인합니다.
- `invalid_checkpoint_change`: 기존 checkpoint 삭제, 존재하지 않는 signal ID, 실행 기록 형식을 확인합니다.
- `invalid_state_change`: 행동에 허용된 생애주기 전환 외 상태가 함께 변경됐는지 확인합니다.
- `invalid_problem_path`: branch 행동의 problem ID, 단일 문제 JSON 경로, 저장된 근거 참조를 확인합니다.
