...
quality workflow는 PR 존재 여부, 현재 저장소, head branch, head SHA를 다시 확인하고 전달된 정확한 SHA를 checkout합니다. 결과는 reusable workflow output으로 부모 workflow에 돌아오며 PR에는 `ready_for_human_review`, `quality_check_failed`, `sha_mismatch`, `quality_check_not_started` 중 하나가 기록됩니다. 결과 기록 후 최종 gate가 실패 결과를 workflow 실패로 반영합니다.
...
수동 실행 결과는 해당 품질검사 실행의 summary와 `quality-check-manual-result` artifact에만 저장됩니다. Agent workflow는 이 artifact를 다운로드하거나 기다리지 않습니다.
