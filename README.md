...
3. Actions에서 **ZeroFounder 에이전트**를 수동 실행하고 `setup-labels`를 선택합니다.
4. 공개 저장소의 무료 Actions·Pages 범위를 권장합니다.

workflow 최상위 권한은 `contents: read`입니다. 모델, Issue, 브랜치, PR, 품질검사, Pages job이 필요한 최소 권한만 따로 받습니다. `pull_request_target`은 사용하지 않습니다.

## Repository Variables

| 이름 | 기본값 | 설명 |
|---|---:|---|
| `OPERATING_LANGUAGE` | `ko` | 창업자가 읽는 운영 산출물 언어. 현재는 한국어를 완전 지원 |
...
1. Actions → **ZeroFounder 시장 신호 수집** → Run workflow를 실행합니다.
2. `signals/raw/`에 새 고유 신호가 생겼는지 확인합니다.
3. Actions → **ZeroFounder 에이전트** → `agent`를 수동 실행합니다.
...
quality workflow는 기본 브랜치의 신뢰된 코드를 `control/`, 검증된 PR head SHA를 `candidate/`에 각각 checkout합니다. PR 존재 여부, open 상태, 현재 저장소, head branch와 SHA가 모두 일치한 뒤에만 candidate를 실행합니다. 제어 helper·결과 집계·상태 기록은 항상 control에서 실행하고 제품 테스트만 candidate에서 실행합니다.

결과는 reusable workflow output으로 부모 workflow에 돌아옵니다. PR에는 `ready_for_human_review`, `quality_check_failed`, `invalid_pr`, `branch_mismatch`, `sha_mismatch`, `repository_mismatch`, `quality_check_not_started` 중 하나가 한국어 설명과 함께 기록됩니다. 결과 기록 후 최종 gate가 실패 결과를 workflow 실패로 반영하며 자동 병합하지 않습니다.

`GITHUB_TOKEN`으로 생성된 PR은 GitHub 보안 정책에 따라 후속 workflow에 인간 승인이 필요할 수 있습니다. 승인 대기는 성공이 아니며 개인 PAT를 기본 해결책으로 사용하지 않습니다.

기존 PR을 새로 만들지 않고 수동 검증하려면 Actions → **ZeroFounder 품질 검사** → Run workflow에서 기존 PR 번호, 정확한 head branch, 40자리 head SHA를 입력합니다. 값은 PR 화면과 다음 명령으로 확인할 수 있습니다.

```bash
gh pr view <PR번호> --json number,headRefName,headRefOid
```

수동 실행 결과는 해당 품질검사 실행의 summary와 `quality-check-manual-result` artifact에 저장되고 PR 본문의 품질검사 상태도 갱신됩니다. Agent workflow는 이 artifact를 다운로드하거나 기다리지 않습니다.

기존 PR #1은 다음 값으로 다시 검사할 수 있습니다. workflow 수정본이 기본 브랜치에 반영된 뒤 Actions → **ZeroFounder 품질 검사** → Run workflow를 열어 입력합니다.

```text
Pull request number: 1
Validated agent branch: agent/29757293892-create-problem-candidate
Exact commit to verify: f92b9cf92296a9c7828450bbe6528701b2064094
```

PR #1을 삭제하거나 새로 만들 필요는 없습니다. 오래된 candidate commit에 최신 helper가 없어도 기본 브랜치의 control helper가 검증과 집계를 담당합니다.

## 운영 언어와 시장 범위

ZeroFounder의 내부 자동화 계약은 영어를 유지합니다. Python·TypeScript 식별자, JSON key, enum value, action type, lifecycle stage, 경로, label 식별 이름, workflow input/output key는 번역하지 않습니다. 창업자가 읽는 PR·Issue·댓글·승인 요청·보고서·평가서·실험·로드맵·Actions 설명은 기본적으로 한국어로 생성합니다.

한국어 출력은 한국 시장 우선 정책이 아닙니다. 시장 탐색은 전 세계와 모든 지원 언어의 공개 자료를 대상으로 하며 국가나 언어 자체에 가산점·감점을 주지 않습니다. 외국어 신호는 원문 언어·제목·짧은 요약·URL과 한국어 제목·요약을 함께 보존합니다. 규칙 기반 수집 단계에서 번역할 수 없으면 원문을 보존하고 번역 대기와 신뢰도 0을 명시합니다.

실제 MVP UI 언어는 운영 언어와 독립적입니다. 목표 지역과 `target_languages`를 기준으로 MVP_PLANNING 또는 INFRASTRUCTURE_SELECTION에서 결정합니다. 예를 들어 글로벌 영어권 제품은 운영 문서가 한국어여도 제품 UI는 영어입니다.

기존 열린 PR과 Issue의 언어 형식은 Actions → **ZeroFounder 에이전트**에서 `migrate-language-dry-run`을 먼저 실행해 확인합니다. 검토 후 `migrate-language-apply`를 수동 실행하면 저장소의 `GITHUB_TOKEN`으로 반영됩니다. 개인 PAT는 필요하지 않습니다. 이 과정은 `agent-generated` 라벨이 있고 봇 또는 GitHub Actions가 작성한 항목만 처리하며 사용자 작성 Issue, PR, 댓글은 수정하지 않습니다.
...
- `quality_check_not_started`: reusable workflow 입력과 PR 존재 여부를 확인합니다.
- `invalid_pr`: PR 번호와 open 상태를 확인합니다. 닫히거나 병합된 PR은 실행하지 않습니다.
- `branch_mismatch`: 입력 branch와 실제 PR head branch를 비교합니다.
- `sha_mismatch`: 입력한 branch·SHA와 실제 PR head가 같은지 확인합니다.
- `repository_mismatch`: PR head와 base 저장소가 현재 저장소인지 확인합니다.
- `quality_check_failed`: PR 본문에 기록된 실패 검사와 품질검사 run URL을 확인합니다.
